...
def train(xtrainp, ytrainp, xvalp, yvalp, 
          ...
          train_analysis = 'train_analysis.csv',train_trace = '', val_analysis = 'val_analysis.csv',val_trace = '',
          model_dir = './model/', tag = ''):
    '''
    Train a model and save off model architecture and weights
    '''
    #f model path exists, delete it
    ...
    history = model.fit(xtrain, ytrain, 
                        ...
                        epochs=epochs , batch_size=bs, callbacks = [es, mc])
    ...
    plt.yticks([0.8,0.825, 0.85, 0.875, 0.9, 0.925, 0.95, 0.975, 1.])
    ...
    #ollect the training labels, predictions, and correctness and output to train analysis
    ...
    return t_accuracy*100, v_accuracy*100
 
#@debugprint
def test(xtest, ytest, test_analysis = 'test_analysis.csv',
          test_trace = '', model_dir = './model/'):
    '''
    ...
    #ollect the training labels, predictions, and correctness and output to val analysis
    #predictions = model.predict(xval)
    #correct = [ y_gt == round(y_pred[0])  for y_gt, y_pred in zip(yval, predictions) ]
    #v_analysis =  [ [y_gt[0],round(y_pred[0]), int(c)]  for y_gt, y_pred, c in zip(yval, predictions, correct)]
    #v_analysis = pd.DataFrame(v_analysis, columns = ['Ground_Truth', 'Predicted', 'Correct'])
