...
def main():
    ...
    if not os.path.exists('./hyp_tuning'):
            os.makedirs('./hyp_tuning')  
            
            
    ###################### Hyperprarmeter tuning section. These values are frequently edited and fine-tuned ##################### 
    hyper_params = {"shape1": 100, "shape2": 31, "shape3": 10,"l2_reg": 0.0003, "epochs": 800,\
            "activation_function": 'relu', "batch_size": 32, "learning_rate": 0.0005 }

    #nested loops to sweep over different values of hyperparameters
    for i in range(2):
        for hyper_param in ["shape3","shape1","shape2","l2_reg","batch_size","learning_rate","activation_function", "epochs"]:
            if hyper_param == "shape1":
                param_value_array = [hyper_params[hyper_param],1000]
            elif hyper_param == "shape2":
                param_value_array = [hyper_params[hyper_param],800]
            elif hyper_param == "shape3":
                param_value_array = [0,10]
            elif hyper_param == "activation_function":
                param_value_array = [hyper_params[hyper_param],'relu', 'tanh', 'sigmoid']
            elif hyper_param == "batch_size": 
                param_value_array = [hyper_params[hyper_param],512, 256, 128]
                ...
                param_value_array = np.array([0.01, 0.003])
    ###########################################################################################################################
            ...
            for param_value in param_value_array:
                ...
                model_dir = './model100k' + model_dir.replace('.', '_')
                total_loop_iter += 1
                ...
                if True:
                    ...
                    hp_df.to_csv( './hyp_tuning/hyperparameter_training_test.csv', index = True, mode = 'a')
                ...
                if validation_accuracy > absolute_max_accuracy_obtained:
                    ...
                    f = open('./hyp_tuning/abs_opt_hyper_param.txt',"w")
